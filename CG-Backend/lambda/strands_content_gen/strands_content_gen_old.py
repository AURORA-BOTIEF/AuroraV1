            ...
            if total_module_lessons <= MAX_LESSONS_PER_BATCH:
                ...
                for batch_idx in range(num_batches):
                    ...
                    if batch_idx > 0:  # First batch already has lock from module-level acquisition
                        ...
                        # Invoke PhaseCoordinator to acquire lock with concurrency tracking
                        ...
                        lock_response = lambda_client.invoke(
                            ...
                            Payload=json.dumps({
                                ...
                                'use_concurrency': True  # Enable concurrent batch tracking
                            })
                        )
                        ...
                        if lock_result.get('statusCode') != 200 or not lock_result.get('can_proceed', False):
                            wait_seconds = lock_result.get('wait_seconds', 60)
                            active_batches = lock_result.get('active_batches', 0)
                            print(f"⏳ Lock not available ({active_batches} batches active), waiting {wait_seconds}s...")
                        ...
                        active = lock_result.get('active_batches', 0)
                        print(f"✅ Lock acquired for batch {batch_idx + 1} (slot {active + 1})")
                    else:
                        # First batch - register it as running
                        batch_phase_name = f"module-{module_num}-batch-1"
                        lambda_client = boto3.client('lambda')
                    ...
                    # Release lock to free up concurrency slot
                    batch_phase_name = f"module-{module_num}-batch-{batch_idx + 1}"
                    lambda_client.invoke(
                        FunctionName='PhaseCoordinator',
                        InvocationType='RequestResponse',
                    ...
                    print(f"🔓 Lock released for batch {batch_idx + 1}")
