...
try:
    ...
    logger.warning("⚠️  vertexai not available (Imagen 4.0 will not work)")
...
DEFAULT_IMAGE_MODEL = 'gemini'  # Options: 'gemini', 'imagen'
...
def generate_image_imagen(model, prompt_id: str, prompt_text: str) -> tuple:
    """
    Generate an image using Imagen 4.0 Ultra.
    ...
    """
    try:
        logger.info(f"Generating image with Imagen for prompt: {prompt_id}")
        logger.info(f"Prompt text length: {len(prompt_text)} characters")
        logger.info(f"Prompt preview: {prompt_text[:150]}{'...' if len(prompt_text) > 150 else ''}")
        ...
        if not images or len(images) == 0:
            logger.info("No images generated by Imagen")
        ...
        if not image_bytes or len(image_bytes) == 0:
            logger.info("Empty image data from Imagen")
        ...
        logger.info(f"Error generating image with Imagen for {prompt_id}: {e}")
        import traceback
        traceback.print_exc()
        return False, None, str(e)
...
def lambda_handler(event: Dict[str, Any], context) -> Dict[str, Any]:
    ...
    try:
        ...
        if image_model == 'imagen':
            logger.info("🎨 Initializing Imagen 4.0 Ultra...")
            
            # Check if Vertex AI is available
            if aiplatform is None or ImageGenerationModel is None:
                logger.error("❌ Vertex AI libraries not available for Imagen")
                return {
                    "statusCode": 500,
                    "error": "Vertex AI libraries not available; cannot use Imagen",
                    "generated_images": [],
                    "bucket": course_bucket,
                    "project_folder": project_folder
                }
            ...
            service_account_json = get_google_service_account()
            if not service_account_json:
                logger.error("❌ Google Service Account not available for Imagen")
                return {
                    "statusCode": 500,
                    "error": "Google Service Account not available; cannot use Imagen",
                    "generated_images": [],
                    "bucket": course_bucket,
                    "project_folder": project_folder
                }
            
            # Write service account to temp file for Vertex AI
            import tempfile
            ...
            try:
                ...
                logger.info(f"✅ Initialized Vertex AI: {GCP_PROJECT_ID} @ {GCP_LOCATION}")
                
                # Load Imagen model
                model = ImageGenerationModel.from_pretrained(IMAGEN_MODEL)
                generate_func = generate_image_imagen
                logger.info(f"✅ Loaded Imagen model: {IMAGEN_MODEL}")
            except Exception as e:
                logger.error(f"❌ Failed to initialize Imagen: {e}")
                return {
                    "statusCode": 500,
                    "error": f"Failed to initialize Imagen: {e}",
                    "generated_images": []
                }
        else:
            # Default to Gemini
