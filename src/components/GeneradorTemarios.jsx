...
const API_BASE = "https://h6ysn7u0t1.execute-api.us-east-1.amazonaws.com/dev2";
...
function GeneradorTemarios() {
  ...
  // 🔹 Estados para versiones guardadas
  ...
  const handleGenerarTemario = async () => {
    ...
    try {
      ...
      console.error("❌ Error al generar el temario:", error);
  ...
  return (
    <div className="contenedor-temario">
      <h2 className="titulo-generador">Generador de Temarios a la Medida</h2>

      <div className="formulario-generador">
        <div className="campo">
                  ...
                  <tbody>
                    {versionesGuardadas.map((v, i) => (
                      <tr key={i}>
                        ...
                        <td>{v.tecnologia}</td>
                        <td>
                          {new Date(v.fecha_guardado).toLocaleString("es-MX")}
                        </td>
                        <td>
                          <button
                            className="btn-abrir"
                            onClick={() => {
                              const parsed = v.temario_json
                                ? JSON.parse(v.temario_json)
                                : v;
